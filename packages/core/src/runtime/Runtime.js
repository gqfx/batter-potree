...
export class Runtime {
   ...
   * ```
   */
  constructor(camera = new PerspectiveCamera(), budgets) {
    /**
     * 渲染运行时配置
     *
     * @description
    ...
    this.budgets = {
      ...
      cpuMemory: budgets?.cpuMemory ?? 1 * 1024 * 1024 * 1024, // 默认 1GB
    };
  }
  /**
   * 检查 GPU 内存预算
   ...
   * ```
   */
  checkGPUMemoryBudget(requiredBytes) {
   ...
   * ```
   */
  evictInvisibleLoadedNodes(resourceManager) {
