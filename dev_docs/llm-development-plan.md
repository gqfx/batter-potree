...
5. 创建顶层目录结构:
   - 创建 tests/{integration,e2e,fixtures,utils}
   - 创建 docs/{api,guides,architecture}
...
- ✅ README 完整
- ✅ 浏览器兼容性验证通过
- ✅ **技术决策**: 评估是否迁移到 bitecs (见 TASK-311)
...
- **TASK-311: ECS 架构评估（bitecs 迁移决策）** ← 新增

详细任务内容参考 `llm-plan-extension.md` 第 ## 🚀 Phase 3 部分

### TASK-311: ECS 架构评估（bitecs 迁移决策）

**任务 ID**: TASK-311
**依赖**: TASK-301 (性能基准测试框架)
**预计时间**: 2 小时
**优先级**: P2

#### 目标
基于性能基准测试结果，评估当前 ECS 实现是否满足架构要求，决定是否需要迁移到 bitecs
...
```
- Phase 3 性能基准测试结果
- architecture-v8.md 第 7 节 ECS 要求
- bitecs 文档和性能数据
```

#### 输出
- [ ] 创建 `dev_docs/ecs-performance-report.md`
- [ ] 包含当前 ECS 性能数据
- [ ] 包含 bitecs 迁移成本评估
- [ ] 明确是否迁移的决策和理由
...
#### LLM Prompt 模板
```
我正在评估 better-potree 项目的 ECS 架构。

任务: 基于性能基准测试结果，决定是否迁移到 bitecs
...
3. 决策标准:
   - 如果当前 ECS 满足所有性能要求 → 保持现状
   - 如果存在性能瓶颈且 bitecs 能解决 → 制定迁移计划
   - 如果迁移成本过高 → 优化现有实现
...
```bash
# 1. 运行 ECS 性能基准
pnpm run test -- benchmarks/ecs

# 2. 检查报告
cat dev_docs/ecs-performance-report.md
...
### Phase 3 (TASK-301 ~ TASK-311)
- **任务数**: 11 个
- **预计时间**: 5 天
- **核心内容**: 性能优化、文档、测试、发布、ECS 架构评估
...
- ✅ 新增 Phase 3 (11 任务) - 从 llm-plan-extension.md
- ✅ 新增项目阶段概览表
- ✅ 新增完整任务统计
- ✅ 新增任务依赖关系图
- ✅ 新增关键里程碑
- ✅ 新增 TASK-311: ECS 架构评估（bitecs 迁移决策）
- ✅ 补充 Phase 2/3 性能退出标准
- ✅ 补充 TASK-101 tests/ 和 docs/ 目录结构创建
- ✅ 总任务数: 53 个
